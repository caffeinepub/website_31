...
export default function ChildSummaryCard({ child, attendance }: ChildSummaryCardProps) {
  return (
      ...
      <CardContent className="p-4">
        <div className="flex items-center gap-4 mb-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">{child.ageMonths} months · {child.gender} · {child.id}</p>
            <div className="mt-1">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="bg-muted rounded-lg p-3 text-center">
            ...
            <p className={`font-bold text-base ${attendance && attendance.percentage < 75 ? 'text-status-yellow' : 'text-status-green'}`}>
        ...
        <div className="mt-3 pt-3 border-t border-border">
          <p className="text-xs text-muted-foreground">
            Parent: <span className="font-medium text-foreground">{child.parentName}</span>
            {' · '}
            <span className="font-medium text-foreground">{child.parentPhone}</span>
          </p>
        </div>
      </CardContent>
    </Card>
