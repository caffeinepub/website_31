...
import { Heart, PlusCircle, Users, AlertTriangle, ShieldCheck, Filter } from 'lucide-react';
...
import PregnantWomanCard, { toCardData, localToCardData } from '../components/PregnantWomanCard';
...
import type { RiskLevelLocal, TrimesterLocal } from '../utils/pregnantWomenData';
...
export default function PregnantWomenPage() {
  ...
  const { data: backendRecords, isLoading, refetch } = useGetAllPregnancyRecords();
  ...
  const filtered = useMemo(() => {
    return allCards.filter((c) => {
      ...
      const matchTrimester =
        trimesterFilter === 'all' || c.trimester === parseInt(trimesterFilter, 10);
    ...
    <div className="space-y-4 animate-slide-up">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">ANC monitoring & maternal health tracking</p>
        </div>
        <Button
          size="sm"
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="bg-card border border-border rounded-xl p-3 shadow-card col-span-2 flex items-center gap-3">
            ...
            <div className="ml-auto flex gap-3 text-center">
              <div>
                <p className="text-sm font-bold text-blue-600">{stats.byTrimester[1]}</p>
                <p className="text-xs text-muted-foreground">1st Tri</p>
              </div>
              <div>
                <p className="text-sm font-bold text-purple-600">{stats.byTrimester[2]}</p>
                <p className="text-xs text-muted-foreground">2nd Tri</p>
              </div>
              <div>
                <p className="text-sm font-bold text-orange-600">{stats.byTrimester[3]}</p>
          ...
          <div className="bg-status-green-bg border border-status-green/20 rounded-xl p-3 shadow-card flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-bold font-heading text-status-green">{stats.byRisk.normal}</p>
          ...
          <div className="bg-status-yellow-bg border border-status-yellow/20 rounded-xl p-3 shadow-card flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-bold font-heading text-status-yellow">{stats.byRisk.moderate}</p>
          ...
          <div className="bg-status-red-bg border border-status-red/20 rounded-xl p-3 shadow-card col-span-2 flex items-center gap-3">
            ...
            <div>
              <p className="text-xl font-bold font-heading text-status-red">{stats.byRisk.high}</p>
              <p className="text-xs text-status-red/80">High Risk — Immediate Attention</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          ...
          <Select value={trimesterFilter} onValueChange={(v) => setTrimesterFilter(v as TrimesterFilter)}>
