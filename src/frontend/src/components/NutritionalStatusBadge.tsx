...
export default function NutritionalStatusBadge({
  status,
  size = "md",
}: NutritionalStatusBadgeProps) {
  const sizeClasses = {
  ...
  const colorClasses = {
    ...
    moderate:
      "bg-status-yellow-bg text-status-yellow border border-status-yellow/30",
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full font-semibold ${sizeClasses[size]} ${colorClasses[status]}`}
    >
