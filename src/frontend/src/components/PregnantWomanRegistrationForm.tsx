  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      await addRecord.mutateAsync({
        name: data.name,
        age: BigInt(Number.parseInt(data.age, 10)),
        ...
        hemoglobinLevel: Number.parseFloat(data.hemoglobinLevel),
        bloodPressure: BigInt(Number.parseInt(data.bloodPressure, 10)),
        ...
        initialVisit: {
          ...
          weight: Number.parseFloat(data.weight),
          bloodPressure: BigInt(Number.parseInt(data.bloodPressure, 10)),
          hemoglobin: Number.parseFloat(data.hemoglobinLevel),
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="col-span-2 space-y-1">
          ...
          />
          {errors.lmpDate && (
            <p className="text-xs text-destructive">{errors.lmpDate.message}</p>
          )}
        </div>
        ...
        {computed && (
          <div className="col-span-2 bg-secondary/50 border border-primary/20 rounded-lg p-3 grid grid-cols-3 gap-2 text-center">
            ...
            <div>
              ...
              <p className="font-bold text-sm text-primary">
                {computed.trimester}
                {computed.trimester === 1
                  ? "st"
                  : computed.trimester === 2
                    ? "nd"
                    : "rd"}
              </p>
            </div>
            <div>
              ...
              <p className="font-bold text-sm text-primary">
                {computed.edd.toLocaleDateString("en-IN", {
                  day: "2-digit",
                  month: "short",
                  year: "numeric",
                })}
        ...
        <div className="space-y-1">
          ...
          {errors.hemoglobinLevel && (
            <p className="text-xs text-destructive">
              {errors.hemoglobinLevel.message}
            </p>
          )}
        </div>
