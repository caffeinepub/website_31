...
export default function GrowthAlertBanner({
  status,
  childName,
}: GrowthAlertBannerProps) {
  ...
  return (
    ...
    >
      {isSevere ? (
        ...
        <AlertTriangle
          className="text-status-yellow mt-0.5 shrink-0"
          size={20}
        />
      )}
      <div>
        <p
          className={`font-semibold text-sm ${isSevere ? "text-status-red" : "text-status-yellow"}`}
        >
          {isSevere
